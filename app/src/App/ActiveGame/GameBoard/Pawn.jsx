import React from 'react'
import { useDrag, DragPreviewImage } from 'react-dnd'

import ItemTypes from './ItemTypes';
import PawnImage from '../../PawnImage.jsx';

const previewImage = 'data:image/gif;base64,'; // TODO :)


export default function Pawn ({ position, name, direction, classList, newProps, isBoard }) {
  const [{ isDragging }, drag, preview] = useDrag({
    item: {
      type: ItemTypes.PAWN,
      newProps: newProps,
      position: position
    },
    collect: monitor => ({
      isDragging: !!monitor.isDragging(),
    }),
  })


  return <>
    <DragPreviewImage connect={preview} src={previewImage} />
    <div ref={drag} style={{
      opacity: isDragging ? 0.5 : 1,
      fontSize: 25,
      cursor: 'move',
    }}>
      <PawnImage name={name} direction={direction} classList={classList} newProps={newProps} isBoard={isBoard} />
    </div>
  </>
}