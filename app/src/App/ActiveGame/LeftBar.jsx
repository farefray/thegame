...
class LeftBar extends Component {
    ...
    if (this.props.stats) {
      ...
      if (s.evolves_to) {
        if (Array.isArray(s.evolves_to)) {
          ...
          for (let i = 0; i < s.evolves_to.length; i++) {
            evoList.push(
              <span key={'evo' + s.evolves_to[i]}>
                <PawnImage name={s.evolves_to[i]} />
              </span>
            );
          }
          evolves_to = (
      ...
      const content = (
        <div className="center">
          ...
          {s.reqEvolve ? (
            <div className="pokemonBabyInfoPanel">
          ...
          <div className="infoPanelStats">
            ...
            <span className="center">
              <span>{`Hp: ${s.hp}`}</span>
              {buffs['hp'] ? <span className="infoPanelBuff">{` + ${buffs['hp']}\n`}</span> : '\n'}
            </span>
            <span>
              <span>{`Attack: ${s.attack}`}</span>
            ...
            <span>
              ...
              {buffs['speed'] ? <span className="infoPanelBuff">{` + ${buffs['speed']}\n`}</span> : '\n'}
            </span>
            <span>
              <span>{`Sp.Attack: ${s.specialAttack}`}</span>
              {buffs['specialAttack'] ? <span className="infoPanelBuff">{` + ${buffs['specialAttack']}\n`}</span> : '\n'}
            </span>
            <span>
              <span>{`Sp.Defense: ${s.specialDefense}`}</span>
      ...
      <div className={className}>
        ...
        {allowSell ? (
          <div className="marginTop5">
            <button
              className="normalButton textList marginLeft5"
              onClick={() => {
                const from = this.props.selectedUnit.pos;
                sellPieceEvent(this.props, from);
              }}
            >
              <span>{'Sell ' + this.props.stats.displayName + '\n'}</span>
  ...
  selectedUnitInformation = () => {
    ...
    const noSelected = (
      <div className={className}>
        <div className={`noSelected`}>No unit selected</div>
      </div>
    );
    if (this.props.stats && !this.props.isSelectModeShop && !isUndefined(this.props.selectedUnit)) {
      let pokemon = this.props.selectedUnit.isBoard
        ? this.props.isActiveBattleGoing && this.props.battleStartBoard
          ? this.props.battleStartBoard[this.props.selectedUnit.pos]
          : this.props.myBoard[this.props.selectedUnit.pos]
        : this.props.myHand[this.props.selectedUnit.pos];
      if (pokemon) {
  ...
  */
  ...
  displayBuffsRender = (boardBuffs, isEnemy = false) => {
    ...
    for (let i = 0; i < sortedBuffKeys.length; i++) {
      ...
      list.push(
        <span key={type} className="typeElement" style={{ marginLeft: left, marginTop: top }} onClick={() => this.props.dispatch({ type: 'SET_MARKED_BUFF', buff: type })}>
    ...
    if (this.props.markedBuff && this.props.displayMarkedBuff && !isEnemy) {
      ...
      if (!isUndefined(req)) {
        ...
        let reqList = [
          <span key={'disp_0' + typeName} className={`${classList}`}>
            {req[0]}
          </span>
        ];
        let bonusAmountList = [
          <span key={'dispValue_0'} className={`${classList}`}>
            {bonusAmount[0]}
          </span>
        ];
        for (let i = 1; i < req.length; i++) {
          ...
          reqList.push(
            <span key={'disp_' + i + '_' + typeName} className={`${classList}`}>
              {', ' + req[i]}
            </span>
          );
          bonusAmountList.push(
            <span key={'dispValue_' + i} className={`${classList}`}>
              {', ' + bonusAmount[i]}
            </span>
          );
        }
        buffInfoDiv = (
          <div className="buffInfoDiv" style={{ marginTop: Math.floor((counter - 1) / 4) * 60 + 60 }}>
  ...
  render() {
    return (
      <div className="leftBar">
        ...
        <div className="boardBuffs text_shadow">{this.props.boardBuffs && this.props.boardBuffs.buffMap && Object.keys(this.props.boardBuffs.buffMap).length > 0 ? this.displayBuffs() : ''}</div>
        <div className="battleEnemyBuffs text_shadow">{this.props.isActiveBattleGoing && !Number.isNaN(this.props.enemyIndex) ? this.displayEnemyBuffs() : ''}</div>
        <div className="flex musicDiv">
          ...
          <img
            className="chatSoundImg"
            src={this.props.chatSoundEnabled ? getImage('chatSound') : getImage('chatSoundMuted')}
            onClick={() => this.props.dispatch({ type: 'TOGGLE_CHAT_SOUND' })}
            alt="chatSoundToggle"
          />
          {this.props.musicEnabled ? this.playMusic() : ''}
        </div>
        <div className="paddingLeft5 marginTop5 text_shadow">
          <input type="range" className="volume-bar" value={this.props.volume * 100} min="0" max="100" step="0.01" onChange={this.handleVolumeChange} />
        </div>
