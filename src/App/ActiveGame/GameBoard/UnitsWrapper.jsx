...
function dispatchUnitLifecycleReducer(unitComponents, action) {
  if (action.type === 'BOARD_UPDATE') {
    const _unitComponents = {}; // todo consider using Map here?
    const { board } = action;
...
function UnitsWrapper({ battleStartBoard, actionStack }) {
  ...
  return Object.keys(unitComponents).map((pos) => {
    ...
    return <Unit key={unit.key} unit={unit} onLifecycle={dispatchUnitLifecycle.bind(this)} />;
  });
}
